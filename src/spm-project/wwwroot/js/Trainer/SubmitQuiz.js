...
$("#create_TFqn").click(function () {
	...
	$("#createQuiz").click(function () {
		...
		jsObjectForm.TimeLimit = $("#quizDuration").val();
		if ($('#gradedQuiz').is(':checked')) {
			jsObjectForm.IsGraded = true;
			jsObjectForm.ChapterId = null;
		} else {
			jsObjectForm.IsGraded = false;
			jsObjectForm.ChapterId = $("#ChapterID").val();
		};
		jsObjectForm.CourseClassId = $("#CourseClassID").val();
		...
		$(".questions").each(function () {
			...
			//Retrieve marks
			questionObject.Marks = $(this).find("#qnMarks").val();
			...
			if ($(this).find('.qnOption').length > 2) {
				...
				if ($(this).find('#multiselectFalse').is(':checked')) {
